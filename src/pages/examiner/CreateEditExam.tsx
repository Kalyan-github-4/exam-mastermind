...
const CreateEditExam: React.FC = () => {
  ...
  const { getExamById, addExam, updateExam } = useExamContext();
  const { toast } = useToast();
  
  const isEditing = !!id;
  const existingExam = isEditing ? getExamById(id) : undefined;
  ...
  const handleSave = () => {
    ...
    const examData: Exam = {
      ...
      status,
      createdAt: isEditing ? existingExam!.createdAt : new Date().toISOString()
    };
