...
import { Clock, FileText, Calendar, ArrowRight, CheckCircle } from 'lucide-react';
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {availableExams.reduce((acc, exam) => acc + exam.questions.length, 0)}
            </div>
          </CardContent>
        </Card>
          ...
          <div className="flex flex-wrap gap-1 pt-2">
            {questionTypeCounts['single-choice'] > 0 && (
              <Badge variant="outline" className="text-xs">
                {questionTypeCounts['single-choice']} Single
              </Badge>
            )}
            {questionTypeCounts['multiple-choice'] > 0 && (
              <Badge variant="outline" className="text-xs">
                {questionTypeCounts['multiple-choice']} Multiple
              </Badge>
            )}
            {questionTypeCounts['true-false'] > 0 && (
              <Badge variant="outline" className="text-xs">
                {questionTypeCounts['true-false']} T/F
              </Badge>
            )}
          </div>
        </div>
